...
exports.handler = async (event) => {
  try {
    ...
    const webhookUrl = process.env.SHEETS_WEBHOOK_URL; // must be /exec
    const token = process.env.SHEETS_TOKEN; // same string as Apps Script SECRET_TOKEN

    if (!webhookUrl || !token) {
      return {
        statusCode: 500,
        body: JSON.stringify({ error: "Missing SHEETS_WEBHOOK_URL or SHEETS_TOKEN" }),
      };
    }
